...
Added:
	...
	- Now noting down the final observation mjd in each band
	- Added a count for the amount of attempts where the last bin was a detection
'''
...
def main():
	...
	'''
	#For real_time, bash script to put in correct command line args automatically?
	if len(sys.argv) != 4:
	...
	modes = ['ZTFDR2', 'simulations', 'fpbot', 'real_time']
	mode = config.loc['mode'][1]
	if ((mode == modes[0]) | (mode == modes[2]) | (mode == modes[3])): #Files are csv files in a folder
	...
	final_verdicts = give_final_verdicts(all_obs, host_dat, min_sep, min_successes, note_missing_host)
...
class ztf_object:
	...
	def load_source_fpbot(self):
		...
		if self.mode == 'real_time':
			...
			#Code copied from fpbot and adapted to work here
...
def give_final_verdicts(obj_list, host_dat, min_sep, min_successes, note_missing_host=False):
...
def final_verdict(obj_loc, host_dat, min_sep, min_successes, note_missing_host=False):
